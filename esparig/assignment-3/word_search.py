...
class Lexicon:
    ...
    """
    def __init__(self, char: str, ends: bool, next_char: List['Lexicon']) -> None:
        self.chr = char
        self.ends = ends
        self.next = next_char
...
def words_from_grid(lexicon: Lexicon, grid: List[List[str]], rows: int, cols: int) -> Set[str]:
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            words_from_grid_helper(current_str, i, j, [[False]*cols]*rows)
    return found_str
